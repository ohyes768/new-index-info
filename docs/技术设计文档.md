...
| 日期 | 版本 | 变更内容 |
|------|------|----------|
| 2026-01-29 | v2.0.0 | 新增 FastAPI 微服务架构、Docker 部署、RESTful API |
...
### v2.0.0 - FastAPI 微服务架构（2026-01-29）

#### 架构升级

**从单体应用到微服务**：
- **v1.x 架构**：命令行工具，单体应用
- **v2.0 架构**：Gateway + 后端服务微服务架构
...
app = FastAPI(title="新股信息 Gateway")
...
@app.get("/api/a-stock")
async def get_a_stock():
    """获取 A股新股信息"""
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                "http://a_stock_service:8001/api/stocks",
                timeout=30.0
            )
...
async def get_hk_stock():
    """获取港股新股信息"""
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                "http://hk_stock_service:8002/api/stocks",
                timeout=30.0
            )
            return response.json()
    except Exception as e:
...
- 服务发现：使用 Docker Compose 的服务名（a_stock_service, hk_stock_service）
- 超时控制：30 秒超时
- 统一错误处理：捕获异常并返回 HTTP 500

#### A股服务设计

**文件结构**：
```
backend/a_stock_service/
├── main.py              # FastAPI 应用入口
├── config.py            # 配置管理
├── models/
│   └── stock.py         # A股数据模型（复用 deploy/Amarket/models.py）
├── services/
│   ├── fetcher.py       # DataFetcher（复用）
│   ├── processor.py     # DataProcessor（复用）
│   └── formatter.py     # MarkdownFormatter（复用）
├── Dockerfile
└── requirements.txt
```
...
async def get_stocks():
    ...
    try:
        ...
        subscribable_stocks = processor.filter_subscribable_stocks(stocks)
        future_stocks = processor.filter_future_unopened_stocks(stocks, future_days=14)
...
**代码复用**：
- 完全复用命令行工具版本的 `DataFetcher`, `DataProcessor`, `MarkdownFormatter`
- 数据模型 `NewStockInfo` 从 `models/stock.py` 导入
- 业务逻辑零改动，只需包装为 FastAPI 端点

#### 港股服务设计

**文件结构**：
```
backend/hk_stock_service/
├── main.py              # FastAPI 应用入口
├── config.py            # 配置管理
├── models/
│   └── stock.py         # 港股数据模型（复用 deploy/Hmarket/hk_models.py）
├── services/
│   ├── fetcher.py       # HKDataFetcher（复用）
│   ├── processor.py     # HKDataProcessor（复用）
│   └── formatter.py     # HKMarkdownFormatter（复用）
├── Dockerfile
└── requirements.txt
```
...
# main.py
from fastapi import FastAPI
from services.fetcher import HKDataFetcher
from services.processor import HKDataProcessor
from services.formatter import HKMarkdownFormatter

app = FastAPI(title="港股新股服务")
...
async def get_stocks():
    ...
    try:
        ...
        # 2. 补充详情（行业、公司简介）
        stocks = fetcher.enrich_stocks_detail(stocks)
...
services:
  ...
  hk_stock_service:
    build: ./backend/hk_stock_service
    expose:
      - "8002"
    environment:
      - MIN_INTERVAL=5
...
**技术要点**：
- 使用 `expose` 而非 `ports`：后端服务只在内部网络可见
- 使用 `networks`：服务间通过服务名互相访问
...
**超时控制**：
- Gateway 到后端服务：30 秒超时
- 数据获取超时：10 秒（A股）、可配置（港股）
...
系统采用**微服务架构**，支持命令行工具和 FastAPI 两种使用方式：

```
new-index-info/
├── backend/                          # FastAPI 后端服务 [v2.0.0新增]
│   ├── gateway/                      # Gateway 服务
│   │   ├── main.py                   # 入口文件
│   │   ├── config.py                 # 配置管理
│   │   ├── utils/
...
│       ├── Dockerfile
│       └── requirements.txt
├── docker/                           # Docker 配置 [v2.0.0新增]
│   ├── docker-compose.yml            # 服务编排
│   └── .env.example                  # 环境变量模板
├── scripts/                          # 启动脚本
│   ├── build.sh                      # 构建镜像 [v2.0.0新增]
│   ├── start.sh                      # 启动服务 [v2.0.0新增]
│   ├── stop.sh                       # 停止服务 [v2.0.0新增]
│   ├── run.bat/sh                    # A股启动脚本（命令行版本）
│   └── hk_run.bat/sh                 # 港股启动脚本（命令行版本）
├── deploy/                           # 保留命令行工具
│   ├── Amarket/                      # A股市场模块
│   │   ├── models.py                 # A股数据模型
│   │   ├── services.py               # A股服务
...
- **双模式支持**：命令行工具 + FastAPI API，满足不同场景
- **代码复用**：FastAPI 服务完全复用命令行工具的业务逻辑
- **微服务独立性**：各服务独立部署、独立扩展
- **向后兼容**：完整保留命令行工具，不影响现有用户
