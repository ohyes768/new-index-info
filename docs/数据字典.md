...
| 日期 | 版本 | 变更内容 |
|------|------|----------|
| 2026-01-29 | v2.0.0 | FastAPI 微服务架构，数据模型无变化 |
| 2026-01-07 | v1.3.0 | 港股详情页补充、A股公司简介数据源优化和格式统一 |
...
- **变更类型**：字段启用
- **数据来源**：新浪财经港股IPO详情页
- **获取方式**：访问 `hk_IPOProfile.php?symbol={股票代码}`，解析HTML表格中的"板块"字段
- **说明**：虽然新浪财经详情页有"板块"字段，但目前数据为空。字段已预留，未来数据更新后会自动填充

**company_intro（公司简介）**：
- **变更类型**：字段启用
- **数据来源**：新浪财经港股IPO详情页
- **获取方式**：访问 `hk_IPOProfile.php?symbol={股票代码}`，解析HTML表格中的"公司简介"字段
- **示例值**："集成电路、计算机软硬件的设计、开发、销售,商务信息咨询..."
- **说明**：完整显示在表格中，无长度限制
...
**company_intro（公司简介）**：
- **变更类型**：数据源优化
- **修改前**：优先使用"机构简介"，否则使用"主营业务"
- **修改后**：直接使用"主营业务"
- **数据来源**：akshare `stock_profile_cninfo()` 接口
- **优化理由**：主营业务字段更完整、更可靠，直接描述公司核心业务
...
- **修改前**：公司简介在表格外的独立段落显示
- **修改后**：公司简介在表格内显示
- **代码变更**：从 `lines.append("**公司简介**:")` 改为 `lines.append(f"| **公司简介** | {stock.company_intro} |")`
- **效果**：与港股保持一致的表格结构
