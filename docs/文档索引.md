...
- [API文档](#api文档)
- [部署指南](#部署指南)
- [FastAPI 部署指南](#fastapi-部署指南) ✅ [v2.0.0新增]
...
- ✅ **自动化推送**：支持钉钉等平台自动推送
- ✅ **FastAPI RESTful API** ✅ [v2.0.0新增] 长期运行的 HTTP 服务，返回 JSON 格式数据
- ✅ **Docker 容器化部署** ✅ [v2.0.0新增] 微服务架构，一键部署
...
#### FastAPI 微服务版本（v2.0）✅ [新增]
- **Web 框架**: FastAPI 0.104.1+
- **服务器**: Uvicorn (ASGI)
- **HTTP 客户端**: httpx（异步）
- **容器化**: Docker + Docker Compose
- **自动化集成**: n8n 2.x (HTTP Request 节点)
...
#### 1. API 概述
- 架构说明：Gateway + 后端服务微服务架构
- 访问方式：推荐通过 Gateway 访问（统一入口）
- 技术栈：FastAPI + Uvicorn + httpx + Docker
...
GET /api/a-stock
```
- 响应：包含 success、market、data、subscribable_count、future_count
- 状态码：200 OK / 503 Service Unavailable / 500 Internal Server Error
- 响应时间：2-5 秒
...
## FastAPI 部署指南 ✅ [v2.0.0新增]

### 文档信息
- **文件名**: [FASTAPI_DEPLOYMENT.md](./FASTAPI_DEPLOYMENT.md)
- **版本**: v2.0.0
- **最后更新**: 2026-01-29
...
#### 1. 已完成的工作
- ✅ 项目架构改造（Gateway + 后端服务）
- ✅ 服务拆分（Gateway、A-Stock、HK-Stock）
- ✅ Docker 配置（Dockerfile、docker-compose.yml）
- ✅ 启动脚本（build.sh、start.sh、stop.sh）
...
- ✅ Gateway + 后端服务微服务架构
- ✅ Docker 容器化部署
- ✅ 部署和运维脚本（build.sh、start.sh、stop.sh）
- ✅ API接口文档

#### 架构调整
- ✅ 微服务架构：Gateway、A股服务、港股服务
- ✅ 代码复用：FastAPI 服务完全复用命令行工具的业务逻辑
- ✅ 双模式支持：命令行工具 + FastAPI API
...
- ✅ 更新文档索引（v2.0.0）
- ✅ 新增 API接口文档
- ✅ 新增 FastAPI 部署指南（FASTAPI_DEPLOYMENT.md）
